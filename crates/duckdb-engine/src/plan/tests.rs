...
fn map_with_lookups_emits_join_chain() {
    ...
    let sql = map_sql(&doc);
    assert!(
        sql.contains("LEFT JOIN \"c\" ON \"o\".\"customer_id\" = \"c\".\"cust_id\""),
        "left join: {}",
        sql
    );
    assert!(
        sql.contains("INNER JOIN \"r\" ON \"o\".\"region_code\" = \"r\".\"code\""),
        "inner join: {}",
        sql
    );
    assert!(
        sql.contains("\"o\".\"id\" AS \"order_id\""),
        "main expr: {}",
        sql
    );
    assert!(
        sql.contains("\"c\".\"name\" AS \"customer_name\""),
        "lookup_1 expr: {}",
        sql
    );
    assert!(
        sql.contains("\"o\".\"amount\" * 1.08 AS \"net\""),
        "arithmetic expr: {}",
        sql
    );
    assert!(
        sql.contains("WHERE \"r\".\"active\" = true"),
        "filter qualified: {}",
        sql
    );
}
...
fn map_without_lookups_is_unchanged() {
    ...
    let sql = map_sql(&doc);
    assert!(
        sql.contains("amount * 1.08 AS \"net\""),
        "strip-prefix path: {}",
        sql
    );
...
fn map_unconfigured_lookup_ref_errors() {
    ...
    assert!(
        err.contains("lookup_1") && err.contains("lookups"),
        "clear error: {}",
        err
    );
}
...
fn cast_honors_on_error_try_vs_hard_cast() {
    ...
    let sql = compile(&try_doc)
        ...
        .clone();
    assert!(
        sql.contains("TRY_CAST"),
        "default onError should TRY_CAST: {}",
        sql
    );
    ...
    let sql = compile(&fail_doc)
        ...
        .clone();
    assert!(
        sql.contains("CAST") && !sql.contains("TRY_CAST"),
        "onError=fail should hard CAST: {}",
        sql
    );
}
...
fn addcol_wraps_expression_in_declared_type() {
    ...
    let sql = compile(&doc)
        ...
        .clone();
    assert!(
        sql.contains("CAST((qty * price) AS BIGINT)"),
        "addcol should cast expr to declared type: {}",
        sql
    );
}
...
fn downstream_ref_to_window_added_column_is_not_rejected() {
    ...
    assert!(
        compile(&p).is_ok(),
        "rownum-added column must not be rejected downstream"
    );
}
...
fn compiles_csv_filter_parquet() {
    ...
    assert!(
        compiled.stages[1]
            .sql
            .contains("CREATE OR REPLACE VIEW \"f1\""),
...
fn filter_with_single_consumer_reject_is_a_lazy_view() {
    ...
    assert!(
        !filter
            .sql
            .contains("CREATE OR REPLACE TABLE \"f1__reject\""),
...
fn materialize_disk_streams_via_parquet() {
    ...
    assert!(
        matches!(
            src.runtime.as_ref(),
            Some(RuntimeSpec::AttachParquetSource(_))
        ),
...
fn materialize_view_duck_source_becomes_lazy_view() {
    ...
    let s = c
        ...
        .expect("src stage");
    assert!(
        s.sql.contains("CREATE OR REPLACE VIEW"),
        "view src must be a VIEW, got: {}",
        s.sql
    );
    assert!(
        !s.sql.contains("CREATE OR REPLACE TABLE"),
        "view src must not be a TABLE: {}",
        s.sql
    );
    assert!(
        !s.sql.contains("DETACH"),
        "view src must keep duckle_src attached: {}",
        s.sql
    );
    assert!(
        s.runtime.is_none(),
        "view src must stay pure-SQL (so the pipeline batches), got a runtime spec"
    );
    ...
    let s2 = c2
        ...
        .expect("src stage");
    assert!(
        s2.sql.contains("CREATE OR REPLACE TABLE"),
        "multi-consumer view stays a TABLE: {}",
        s2.sql
    );
    ...
    let s3 = c3
        ...
        .expect("src stage");
    assert!(
        s3.sql.contains("CREATE OR REPLACE TABLE"),
        "auto multi-consumer stays a TABLE: {}",
        s3.sql
    );
}
...
fn duck_source_auto_single_consumer_is_a_live_view() {
    ...
    assert!(
        s.sql.contains("CREATE OR REPLACE VIEW"),
        "auto single-consumer must be a VIEW: {}",
        s.sql
    );
    assert!(
        !s.sql.contains("DETACH"),
        "view src keeps its alias attached: {}",
        s.sql
    );
...
fn two_duck_sources_each_stay_views_with_distinct_aliases() {
    ...
    assert!(
        a.sql.contains("CREATE OR REPLACE VIEW") && b.sql.contains("CREATE OR REPLACE VIEW"),
        "both sources must be VIEWs:\nA: {}\nB: {}",
        a.sql,
        b.sql
    );
    assert!(
        a.sql.contains("AS duckle_src_s1"),
        "source A uses its own alias: {}",
        a.sql
    );
    assert!(
        b.sql.contains("AS duckle_src_s2"),
        "source B uses its own alias: {}",
        b.sql
    );
    assert!(
        !a.sql.contains("AS duckle_src_s2") && !b.sql.contains("AS duckle_src_s1"),
        "aliases must not cross"
    );
    assert!(
        !a.sql.contains("DETACH") && !b.sql.contains("DETACH"),
        "both kept attached for the batched session"
    );
}
...
fn merge_resolves_columns_through_a_schemaless_transform() {
    ...
    assert!(
        sink.sql.contains("MERGE INTO"),
        "sink must build a MERGE: {}",
        sink.sql
    );
    assert!(
        sink.sql.contains("\"amount\" = src.\"amount\""),
        "merge must set the non-key source column: {}",
        sink.sql
    );
}
...
fn sqlserver_sink_bulk_uses_mssql_extension() {
    ...
    assert!(
        k.sql.contains("INSTALL mssql FROM community")
            && k.sql.contains("AS duckle_dst (TYPE mssql)"),
    ...
    assert!(
        k.runtime.is_none(),
        "bulk sink is pure SQL (no driver spec)"
    );
    ...
    assert!(
        matches!(k2.runtime.as_ref(), Some(RuntimeSpec::SqlserverSink(_))),
        "bulk=false uses the driver"
    );
    assert!(
        !k2.sql.contains("mssql"),
        "driver path has no mssql ATTACH: {}",
        k2.sql
    );
}
...
fn relational_source_infers_custom_sql_without_mode() {
    ...
    let tbl = build_relational_source(
        "src.quack",
        &json!({"tableName":"orders","schemaName":"main"}),
    )
    .unwrap();
    assert!(
        tbl.starts_with("SELECT * FROM"),
        "table read still works: {}",
        tbl
    );
    assert!(
...
fn ducklake_source_time_travel_read() {
    ...
    use serde_json::json;
    let v = build_relational_source(
        "src.ducklake",
        &json!({"tableName":"orders","asOfVersion":3}),
    )
    .unwrap();
    assert!(v.contains("AT (VERSION => 3)"), "version read: {}", v);
    let vs = build_relational_source(
        "src.ducklake",
        &json!({"tableName":"orders","asOfVersion":"5"}),
    )
    .unwrap();
    assert!(
        vs.contains("AT (VERSION => 5)"),
        "string version read: {}",
        vs
    );
    let t = build_relational_source(
        "src.ducklake",
        &json!({"tableName":"orders","asOfTimestamp":"2026-01-01 00:00:00"}),
    )
    .unwrap();
    assert!(
        t.contains("AT (TIMESTAMP => '2026-01-01 00:00:00')"),
        "timestamp read: {}",
        t
    );
    ...
    let pg = build_relational_source(
        "src.postgres",
        &json!({"tableName":"orders","asOfVersion":3}),
    )
    .unwrap();
...
fn ducklake_diff_builds_change_feed_between_versions() {
    ...
    use serde_json::json;
    let body = build_ducklake_diff(
        &json!({"schema":"main","table":"orders","fromVersion":2,"toVersion":5}),
    );
    assert_eq!(
        body,
        "SELECT * FROM ducklake_table_changes('duckle_src', 'main', 'orders', 2, 5)"
    );
    // string versions + default schema (main)
    let b2 = build_ducklake_diff(&json!({"table":"orders","fromVersion":"1","toVersion":"3"}));
    assert_eq!(
        b2,
        "SELECT * FROM ducklake_table_changes('duckle_src', 'main', 'orders', 1, 3)"
    );
}
...
fn diffsummary_reduces_change_feed() {
    ...
    assert!(
        sql.contains("FILTER (WHERE \"change_type\" = 'insert') AS added"),
        "got: {}",
        sql
    );
    assert!(
        sql.contains("(added + removed + updated) AS total_changes"),
        "got: {}",
        sql
    );
    ...
    assert!(
        custom.contains("FILTER (WHERE \"op\" = 'delete') AS removed"),
        "got: {}",
        custom
    );
}
...
fn materialize_duckdbfile_carries_path_and_requires_it() {
    ...
    assert!(
        err.to_string().contains("materializePath")
...
fn printf_escapes_stray_percent_but_keeps_specs() {
    ...
    assert_eq!(
        escape_stray_printf_percents("val %s (100%%)"),
        "val %s (100%%)"
    );
    assert_eq!(
        escape_stray_printf_percents("50% off %d items"),
        "50%% off %d items"
    );
...
fn skip_orderby_makes_offset_deterministic() {
    ...
    let sql = build_take(
        &ni,
        &serde_json::json!({"count": 5, "orderBy": ["id"]}),
        TakeKind::Offset,
    )
    .unwrap();
    assert!(
...
fn setop_realigns_columns_by_name_without_invalid_syntax() {
    ...
    assert!(
        !sql.contains("INTERSECT BY NAME"),
        "must not emit invalid INTERSECT BY NAME, got: {}",
        sql
    );
    assert!(
        sql.contains(" INTERSECT "),
        "must join legs with plain INTERSECT, got: {}",
        sql
    );
    assert!(
        sql.contains("WHERE false UNION ALL BY NAME"),
        "must realign later legs by name, got: {}",
        sql
    );
    let ex = build_setop(&ni, "EXCEPT").unwrap();
    assert!(
        ex.contains(" EXCEPT ") && !ex.contains("EXCEPT BY NAME"),
        "got: {}",
        ex
    );
}
...
fn cast_per_column_format_uses_strptime() {
    ...
    assert!(
        sql.contains("TRY_CAST(\"amount\""),
        "non-date cast keeps TRY_CAST, got: {}",
        sql
    );
    // A date cast WITHOUT a format still uses TRY_CAST (no regression).
    let no_fmt = build_cast(
        &ni,
        &serde_json::json!({"casts":[{"column":"d","targetType":"date"}]}),
    )
    .unwrap();
    assert!(
        no_fmt.contains("TRY_CAST(\"d\" AS DATE)"),
        "got: {}",
        no_fmt
    );
}
...
fn mask_builds_replace_per_mode() {
    ...
    assert!(
        sql.contains("right(CAST(\"ssn\" AS VARCHAR), 4)"),
        "got: {}",
        sql
    );
    assert!(
        sql.contains("md5('s7' || CAST(\"email\" AS VARCHAR)) AS \"email\""),
        "got: {}",
        sql
    );
    ...
    assert!(
        nosalt.contains("md5(CAST(\"x\" AS VARCHAR))"),
        "got: {}",
        nosalt
    );
...
fn survivor_builds_golden_record_groupby() {
    let mut ni = NodeInputs::default();
    ni.ports.insert("main".into(), vec!["up".into()]);
    let freq = build_survivor(
        &ni,
        &serde_json::json!({"groupBy":["id"],"rule":"most_frequent"}),
    )
    .unwrap();
    assert!(
        freq.contains("mode(COLUMNS(* EXCLUDE (\"id\")))"),
        "got: {}",
        freq
    );
    assert!(freq.contains("GROUP BY \"id\""), "got: {}", freq);
    let recent = build_survivor(
        &ni,
        &serde_json::json!({"groupBy":["id"],"rule":"most_recent","recencyColumn":"updated_at"}),
    )
    .unwrap();
    assert!(
        recent.contains("arg_max(COLUMNS(* EXCLUDE (\"id\")), \"updated_at\")"),
        "got: {}",
        recent
    );
    // most_recent without a recency column is a loud error; unknown rule too.
    assert!(build_survivor(
        &ni,
        &serde_json::json!({"groupBy":["id"],"rule":"most_recent"})
    )
    .is_err());
...
fn matchgroup_builds_recursive_cluster_sql() {
    ...
    assert!(
        sql.contains("CAST(\"id_a\" AS VARCHAR) AS s, CAST(\"id_b\" AS VARCHAR) AS t"),
        "got: {}",
        sql
    );
    assert!(
        sql.contains("SELECT CAST(\"id_b\" AS VARCHAR), CAST(\"id_a\" AS VARCHAR)"),
        "got: {}",
        sql
    );
    assert!(
        sql.contains("SELECT id, MIN(rep) AS cluster_id FROM reach GROUP BY id"),
        "got: {}",
        sql
    );
    assert!(sql.contains("FROM \"up\""), "got: {}", sql);
    let custom = build_matchgroup(
        &ni,
        &serde_json::json!({"leftKey":"left rec","rightKey":"right rec"}),
    )
    .unwrap();
    assert!(
...
fn sample_adv_builds_using_sample_clause() {
    let mut ni = NodeInputs::default();
    ni.ports.insert("main".into(), vec!["up".into()]);
    let seeded = build_sample_adv(
        &ni,
        &serde_json::json!({ "percent": 10, "method": "reservoir", "seed": 42 }),
    )
    .unwrap();
    assert_eq!(
        seeded, "SELECT * FROM \"up\" USING SAMPLE 10 PERCENT (reservoir, 42)",
        "got: {}",
        seeded
    );
    let no_seed = build_sample_adv(&ni, &serde_json::json!({ "percent": 5 })).unwrap();
    assert_eq!(
        no_seed, "SELECT * FROM \"up\" USING SAMPLE 5 PERCENT (reservoir)",
        "got: {}",
        no_seed
    );
    assert!(build_sample_adv(&ni, &serde_json::json!({})).is_err());
    assert!(build_sample_adv(&ni, &serde_json::json!({ "percent": 150 })).is_err());
    assert!(build_sample_adv(
        &ni,
        &serde_json::json!({ "percent": 10, "method": "bogus" })
    )
    .is_err());
...
fn expect_builds_scorecard_per_rule() {
    ...
    assert!(
        sql.contains("COUNT(*) FILTER (WHERE NOT (\"email\" IS NOT NULL))"),
        "got: {}",
        sql
    );
    assert!(
        sql.contains("COUNT(*) FILTER (WHERE NOT (\"amt\" BETWEEN 0 AND 10))"),
        "got: {}",
        sql
    );
    assert!(
        sql.contains("\"status\" IN ('paid', 'pending')"),
        "got: {}",
        sql
    );
    assert!(
        sql.contains("regexp_full_match(CAST(\"code\" AS VARCHAR), '^[A-Z]+$')"),
        "got: {}",
        sql
    );
    assert!(
        sql.contains("COUNT(*) OVER (PARTITION BY \"id\") = 1"),
        "got: {}",
        sql
    );
    assert!(
        sql.contains("'in_set(status, 2 values)' AS expectation"),
        "got: {}",
        sql
    );
    assert!(sql.contains("FROM \"up\""), "got: {}", sql);
    ...
    assert!(build_expect(
        &ni,
        &serde_json::json!({ "rules": [{ "column": "x", "check": "bogus" }] })
    )
    .is_err());
    let kv = build_expect(
        &ni,
        &serde_json::json!({ "rules": { "amt": "in_range:0,10", "id": "unique" } }),
    )
    .unwrap();
    ...
    assert!(
        kv.contains("COUNT(*) OVER (PARTITION BY \"id\") = 1"),
        "got: {}",
        kv
    );
}
...
fn profile_adv_builds_metric_value_long_form() {
    ...
    assert!(
        sql.contains("SELECT CAST(\"email\" AS VARCHAR) AS v FROM \"up\""),
        "got: {}",
        sql
    );
    assert!(
        sql.contains("approx_count_distinct(v) AS distinct_n"),
        "got: {}",
        sql
    );
    assert!(
        sql.contains("regexp_full_match(v, '^-?[0-9]+$')"),
        "int pattern: {}",
        sql
    );
    assert!(
        sql.contains("GROUP BY v ORDER BY freq DESC, v LIMIT 3"),
        "top-n: {}",
        sql
    );
    assert!(
        sql.contains("SELECT \"metric\", \"value\", \"count\", \"pct\" FROM ("),
        "shape: {}",
        sql
    );
    ...
    assert!(
        build_profile_adv(&ni, &serde_json::json!({ "column": "x", "topN": 99999 }))
            .unwrap()
            .contains("LIMIT 1000")
    );
    assert!(build_profile_adv(&ni, &serde_json::json!({})).is_err());
    assert!(build_profile_adv(
        &NodeInputs::default(),
...
fn link_builds_cross_join_over_two_inputs() {
    ...
    ni.ports.insert("main".into(), vec!["m1".into()]);
    ni.ports.insert("lookup".into(), vec!["r1".into()]);
    let sql = build_record_link(
        &ni,
        &serde_json::json!({ "leftKey": "name", "rightKey": "company" }),
    )
    .unwrap();
    assert!(
        sql.contains("FROM \"m1\"") && sql.contains("FROM \"r1\""),
        "got: {}",
        sql
    );
    assert!(sql.contains("CROSS JOIN"), "got: {}", sql);
    assert!(
        sql.contains("jaro_winkler_similarity(a._key, b._key)"),
        "got: {}",
        sql
    );
    assert!(sql.contains(">= 0.85"), "got: {}", sql);
    assert!(
        sql.contains("a._show AS left_key") && sql.contains("b._show AS right_key"),
        "got: {}",
        sql
    );
    ...
    assert!(
        lev.contains("concat_ws(' ', \"first\", \"last\")"),
        "got: {}",
        lev
    );
    let mut no_ref = NodeInputs::default();
    no_ref.ports.insert("main".into(), vec!["m1".into()]);
    assert!(
        build_record_link(&no_ref, &serde_json::json!({"leftKey":"a","rightKey":"b"})).is_err()
    );
...
fn reconcile_builds_full_outer_join_report() {
    ...
    ni.ports.insert("main".into(), vec!["m1".into()]);
    ni.ports.insert("lookup".into(), vec!["r1".into()]);
    let sql = build_reconcile(
        &ni,
        &serde_json::json!({ "keyColumns": ["id"], "measureColumns": ["amount"] }),
    )
    .unwrap();
    assert!(
        sql.contains(
            "FULL OUTER JOIN \"__r\" ON \"__m\".\"id\" IS NOT DISTINCT FROM \"__r\".\"id\""
        ),
        "got: {}",
        sql
    );
    ...
    let composite =
        build_reconcile(&ni, &serde_json::json!({ "keyColumns": ["region", "id"] })).unwrap();
    ...
    assert!(
        !composite.contains("_source_sum"),
        "no measures means no sum metrics: {}",
        composite
    );
...
fn classify_builds_pii_report_sql() {
    ...
    assert!(
        all.contains("SELECT COLUMNS(*)::VARCHAR FROM \"up\""),
        "got: {}",
        all
    );
    assert!(
        all.contains("UNPIVOT INCLUDE NULLS (col_val FOR col_name IN (COLUMNS(*)))"),
        "got: {}",
        all
    );
    assert!(
        all.contains("n_email / NULLIF(sample_count, 0) AS r_email"),
        "got: {}",
        all
    );
    assert!(
        all.contains("GREATEST(COALESCE(r_email, 0)"),
        "got: {}",
        all
    );
    assert!(
        all.contains("CASE WHEN best_rate < 0.8 THEN 'text'"),
        "got: {}",
        all
    );
    assert!(
        all.contains("WHEN r_email = best_rate THEN 'email'"),
        "got: {}",
        all
    );
    assert!(
        all.contains(
            "detected_type IN ('email', 'ssn', 'uuid', 'ipv4', 'credit_card', 'phone') AS is_pii"
        ),
        "got: {}",
        all
    );
    ...
    assert!(
        some.contains("CAST(\"email\" AS VARCHAR) AS \"email\""),
        "got: {}",
        some
    );
    assert!(
        !some.contains("COLUMNS(*)::VARCHAR"),
        "explicit columns must not melt all: {}",
        some
    );
    let clamp = build_classify(&ni, &serde_json::json!({"threshold": 5})).unwrap();
    assert!(
        clamp.contains("CASE WHEN best_rate < 1 THEN 'text'"),
        "got: {}",
        clamp
    );
...
fn rename_from_mapping_file_json_csv() {
    ...
    std::fs::File::create(&jpath)
        .unwrap()
        .write_all(br#"{"a":"alpha","b":"beta"}"#)
        .unwrap();
    let sql = build_rename(
        &ni,
        &serde_json::json!({ "mappingFile": jpath.to_string_lossy() }),
    )
    .unwrap();
    assert!(
        sql.contains("\"a\" AS \"alpha\"") && sql.contains("\"b\" AS \"beta\""),
        "got: {}",
        sql
    );
    ...
    std::fs::File::create(&cpath)
        ...
        .write_all(b"old,new\nx,ex\ny,why\n")
        .unwrap();
    let csv = build_rename(
        &ni,
        &serde_json::json!({ "mappingFile": cpath.to_string_lossy() }),
    )
    .unwrap();
    assert!(
        csv.contains("\"x\" AS \"ex\"") && csv.contains("\"y\" AS \"why\""),
        "got: {}",
        csv
    );
    ...
    assert!(build_rename(
        &ni,
        &serde_json::json!({ "mappingFile": "/no/such/file.json" })
    )
    .is_err());
...
fn csv_extra_read_options_and_filename() {
    ...
    let plain = build_csv_source(
        &serde_json::json!({ "path": "d.csv", "hasHeader": true }),
        None,
    );
...
fn contract_builds_gated_passthrough() {
    ...
    assert!(
        sql.contains("WITH _duckle_contract AS MATERIALIZED"),
        "got: {}",
        sql
    );
    assert!(
        sql.contains("error('Data contract violated: '"),
        "got: {}",
        sql
    );
    assert!(
...
fn surrogate_key_builds_hash_and_sequence() {
    let mut ni = NodeInputs::default();
    ni.ports.insert("main".into(), vec!["up".into()]);
    let hash = build_surrogate_key(
        &ni,
        &serde_json::json!({"mode":"hash","keyColumns":["company","country"]}),
    )
    .unwrap();
    ...
    assert!(
        sep.contains("md5(concat_ws('-', CAST(\"id\" AS VARCHAR))) AS \"dim_key\""),
        "got: {}",
        sep
    );
    let seq = build_surrogate_key(
        &ni,
        &serde_json::json!({"mode":"sequence","keyColumns":["company","country"]}),
    )
    .unwrap();
    assert!(
    ...
    assert!(build_surrogate_key(
        &ni,
        &serde_json::json!({"mode":"bogus","keyColumns":["id"]})
    )
    .is_err());
}
...
fn bucketize_labeled_bounds_mode() {
    ...
    let sql = build_bucketize(
        &ni,
        &serde_json::json!({"column":"age","bounds":[18,40,65]}),
    )
    .unwrap();
    assert!(
        sql.contains("CASE WHEN \"age\" IS NULL THEN NULL"),
        "got: {}",
        sql
    );
    ...
    let lab = build_bucketize(
        &ni,
        &serde_json::json!({"column":"age","bounds":"18,65","labels":["minor","adult","senior"]}),
    )
    .unwrap();
    assert!(
        lab.contains("THEN 'minor'")
            && lab.contains("THEN 'adult'")
            && lab.contains("ELSE 'senior'"),
        "got: {}",
        lab
    );
    // wrong label count is a loud error; without bounds the equal-width path still needs low/high.
    assert!(build_bucketize(
        &ni,
        &serde_json::json!({"column":"age","bounds":[18],"labels":["a","b","c"]})
    )
    .is_err());
    let eqw = build_bucketize(
        &ni,
        &serde_json::json!({"column":"age","low":0,"high":100,"buckets":4}),
    )
    .unwrap();
    assert!(
        eqw.contains("floor(") && !eqw.contains("'<"),
...
fn scd3_keeps_previous_value_in_sibling_column() {
    ...
    let multi = build_scd3(
        &ni,
        &serde_json::json!({ "keyColumns": ["region", "id"], "trackColumns": ["name", "score"] }),
    )
    .unwrap();
    assert!(
        multi.contains("p.\"name\" AS \"previous_name\"")
            && multi.contains("p.\"score\" AS \"previous_score\""),
        "got: {}",
        multi
    );
    assert!(
        multi.contains("p.\"region\" = c.\"region\" AND p.\"id\" = c.\"id\""),
        "got: {}",
        multi
    );
    assert!(
        !multi.contains("CURRENT_TIMESTAMP"),
        "no effective date when unset: {}",
        multi
    );
...
fn qa_outlier_emits_iqr_and_zscore_pass_reject_sql() {
    ...
    let iqr_pass = build_outlier(
        &mk(),
        &serde_json::json!({"column": "amount", "method": "iqr"}),
        false,
    )
    .unwrap();
    assert!(
        iqr_pass.contains("quantile_cont") && iqr_pass.contains("1.5"),
        "got: {}",
        iqr_pass
    );
    assert!(iqr_pass.contains("\"amount\" IS NULL"), "got: {}", iqr_pass);
    assert!(
        iqr_pass.contains("EXCLUDE (__dq_q1, __dq_q3)"),
        "got: {}",
        iqr_pass
    );
    assert!(
        iqr_pass.contains("COALESCE") && !iqr_pass.contains("NOT COALESCE"),
        "got: {}",
        iqr_pass
    );
    let iqr_rej = build_outlier(
        &mk(),
        &serde_json::json!({"column": "amount", "method": "iqr"}),
        true,
    )
    ...
    let z_pass = build_outlier(
        &mk(),
        &serde_json::json!({"column": "amount", "method": "zscore"}),
        false,
    )
    .unwrap();
    assert!(
    ...
    assert!(build_outlier(
        &mk(),
        &serde_json::json!({"column": "amount", "sensitivity": 0}),
        false
    )
    .is_err());
}
...
fn sessionize_builds_gap_window_sql() {
    let mut ni = NodeInputs::default();
    ni.ports.insert("main".into(), vec!["up".into()]);
    let sql = build_sessionize(
        &ni,
        &serde_json::json!({"partitionBy": ["user_id"], "orderBy": "ts", "gap": 30}),
    )
    .unwrap();
    ...
    assert!(
        sql.contains("WINDOW w AS (PARTITION BY \"user_id\" ORDER BY \"ts\")"),
        "got: {}",
        sql
    );
    assert!(
        sql.contains(
            "SUM(__new_sess) OVER (PARTITION BY \"user_id\" ORDER BY \"ts\") AS \"session_id\""
        ),
        "got: {}",
        sql
    );
    assert!(sql.contains("ROW_NUMBER() OVER (PARTITION BY \"user_id\", \"session_id\" ORDER BY \"ts\") AS \"session_seq\""), "got: {}", sql);
    let hrs = build_sessionize(
        &ni,
        &serde_json::json!({ "orderBy": "ts", "gap": 1, "gapUnit": "hours", "emitSeq": false }),
    )
    .unwrap();
    assert!(
        hrs.contains("> 3600")
            && hrs.ends_with("SELECT * FROM __sid")
            && !hrs.contains("session_seq"),
        "got: {}",
        hrs
    );
    ...
    assert!(build_sessionize(
        &ni,
        &serde_json::json!({ "orderBy": "ts", "gap": 5, "gapUnit": "weeks" })
    )
    .is_err());
}
...
fn freshness_builds_gate_and_report() {
    let mut ni = NodeInputs::default();
    ni.ports.insert("main".into(), vec!["up".into()]);
    let gate = build_freshness(
        &ni,
        &serde_json::json!({ "column": "ts", "maxAge": 24, "maxAgeUnit": "hours" }),
    )
    .unwrap();
    assert!(
        gate.contains("WITH _duckle_freshness AS MATERIALIZED")
            && gate.contains("SELECT u.* FROM \"up\" u"),
        "got: {}",
        gate
    );
    assert!(
        gate.contains(
            "date_diff('hour', MAX(CAST(\"ts\" AS TIMESTAMP)), CURRENT_TIMESTAMP) <= 24 THEN 'ok'"
        ),
        "got: {}",
        gate
    );
    assert!(gate.contains("error('Data is stale: '"), "got: {}", gate);
    let report = build_freshness(
        &ni,
        &serde_json::json!({ "column": "ts", "maxAge": 2, "maxAgeUnit": "days", "mode": "report" }),
    )
    .unwrap();
    assert!(
        report.contains(
            "date_diff('day', MAX(CAST(\"ts\" AS TIMESTAMP)), CURRENT_TIMESTAMP) AS age_days"
        ),
        "got: {}",
        report
    );
    ...
    assert!(build_freshness(
        &ni,
        &serde_json::json!({ "column": "ts", "maxAge": 1, "mode": "bogus" })
    )
    .is_err());
}
...
fn attach_prelude_loads_spatial_for_sql_template() {
    ...
    let auto = attach_prelude(
        "code.sqltemplate",
        &serde_json::json!({ "sql": "SELECT ST_Point(lon,lat) FROM input" }),
    );
    assert!(auto.contains("LOAD spatial"), "auto-detect: {}", auto);
    let none = attach_prelude(
        "code.sql",
        &serde_json::json!({ "sql": "SELECT list_value(a), first_name FROM input" }),
    );
    assert!(
        !none.contains("spatial"),
        "must not false-fire on list_/first_: {}",
        none
    );
}
...
fn csv_date_format_passes_through_to_reader() {
    ...
    assert!(
        sql.contains("dateformat='%d/%m/%Y'"),
        "missing dateformat: {}",
        sql
    );
    assert!(
        sql.contains("timestampformat='%d/%m/%Y %H:%M:%S'"),
        "missing timestampformat: {}",
        sql
    );
}
...
fn csv_per_column_format_wraps_with_try_strptime() {
    ...
    assert!(
        sql.contains("'d1': 'VARCHAR'"),
        "d1 not forced VARCHAR: {}",
        sql
    );
    assert!(
        sql.contains("'ts': 'VARCHAR'"),
        "ts not forced VARCHAR: {}",
        sql
    );
    assert!(
        sql.contains("FROM read_csv_auto("),
        "missing reader: {}",
        sql
    );
}
...
fn csv_date_column_without_format_keeps_native_type() {
    ...
    assert!(
        ...
        "should not wrap without format: {}",
        sql
    );
    assert!(
        sql.contains("'d': 'DATE'"),
        "date type not preserved: {}",
        sql
    );
}
...
fn csv_mixed_format_and_plain_columns() {
    ...
    assert!(
        sql.contains("try_strptime(\"d\", '%d/%m/%Y')::DATE AS \"d\""),
        "missing d: {}",
        sql
    );
    assert!(
        !sql.contains("\"n\")") && !sql.contains("AS \"n\""),
        "n should not be rewritten: {}",
        sql
    );
    assert!(
        sql.contains("'n': 'BIGINT'"),
        "int type not preserved: {}",
        sql
    );
}
...
fn cast_with_valid_column_in_schema_compiles() {
    ...
    let cast_sql = compiled
        ...
        .as_str();
    assert!(
        cast_sql.contains("CAST(\"amount\" AS DOUBLE)"),
        "wrong cast SQL: {}",
        cast_sql
    );
}
...
fn cast_with_all_empty_columns_errors_loudly() {
    ...
    assert!(
        msg.contains("no column name"),
        "should mention the empty-column gap: {}",
        msg
    );
}
...
fn cast_with_duplicate_columns_errors_loudly() {
    ...
    assert!(
        msg.contains("'amount'"),
        "should name the duplicate column: {}",
        msg
    );
}
...
fn window_without_order_by_errors_clearly() {
    ...
    let err = compile(&p)
        .err()
        .expect("expected an Err from missing ORDER BY");
    let msg = format!("{:?}", err);
    assert!(
...
fn union_uses_by_name_to_dodge_positional_silent_corruption() {
    ...
    let union_sql = compiled
        ...
        .as_str();
    assert!(
        union_sql.contains("UNION ALL BY NAME"),
        "expected BY NAME variant: {}",
        union_sql
    );
}
...
fn arr_contains_is_null_safe() {
    ...
    let sql = compiled
        ...
        .as_str();
    assert!(
        sql.contains("COALESCE(list_contains"),
        "missing COALESCE shield: {}",
        sql
    );
...
fn join_with_same_key_name_uses_using_clause() {
    ...
    let join_sql = compiled
        ...
        .as_str();
    assert!(
        join_sql.contains("USING (\"customer_id\")"),
        "missing USING clause: {}",
        join_sql
    );
    assert!(
        !join_sql.contains("m.\"customer_id\" = r.\"customer_id\""),
        "should have used USING not ON: {}",
        join_sql
    );
}
...
fn join_with_different_key_names_excludes_right_key() {
    ...
    let join_sql = compiled
        ...
        .as_str();
    assert!(
        join_sql.contains("EXCLUDE (\"cust_id\")"),
        "missing EXCLUDE: {}",
        join_sql
    );
    assert!(
        join_sql.contains("m.\"customer_id\" = r.\"cust_id\""),
        "missing ON clause: {}",
        join_sql
    );
...
fn semi_join_uses_exists_not_in() {
    ...
    let join_sql = compiled
        ...
        .as_str();
    assert!(
        join_sql.contains("NOT EXISTS"),
        "anti should use NOT EXISTS: {}",
        join_sql
    );
    assert!(
        !join_sql.contains("NOT IN"),
        "should not emit NOT IN: {}",
        join_sql
    );
}
...
fn row_hash_emits_concat_ws_with_casts() {
    ...
    assert!(
        sql.contains("CAST(\"id\" AS VARCHAR)"),
        "id not cast: {}",
        sql
    );
    assert!(
        sql.contains("CAST(\"email\" AS VARCHAR)"),
        "email not cast: {}",
        sql
    );
    assert!(
        sql.contains(" AS \"fp\""),
        "custom output column not honoured: {}",
        sql
    );
}
...
fn row_hash_default_algorithm_is_md5() {
    ...
    assert!(
        sql.contains(" AS \"_row_hash\""),
        "default output column wrong: {}",
        sql
    );
}
...
fn audit_emits_selected_columns_only() {
    ...
    let sql = &compiled.stages[1].sql;
    assert!(
        sql.contains("current_timestamp AS _loaded_at"),
        "loaded_at missing: {}",
        sql
    );
    assert!(
        !sql.contains("_loaded_date"),
        "loaded_date should be off: {}",
        sql
    );
    assert!(
        sql.contains("'orders_etl' AS _source"),
        "source literal missing: {}",
        sql
    );
    assert!(
        sql.contains("'2026-05-27' AS _batch_id"),
        "batch_id missing: {}",
        sql
    );
}
...
fn fill_constant_string_value_quoted() {
    ...
    let sql = &compiled.stages[1].sql;
    assert!(
        sql.contains("COALESCE(\"status\", 'unknown')"),
...
fn fill_constant_numeric_value_unquoted() {
    ...
    let sql = &compiled.stages[1].sql;
    assert!(
        sql.contains("COALESCE(\"qty\", 0)"),
        "numeric literal got quoted: {}",
        sql
    );
}
...
fn csv_reject_and_split_partition_bad_rows() {
    ...
    assert!(
        reject.contains("'order_date': 'VARCHAR'"),
        "reject reads raw text: {reject}"
    );
    assert!(
...
fn parquet_sink_forwards_row_group_size() {
    ...
    assert!(
        sql.contains("ROW_GROUP_SIZE 1000000"),
        "row group size not forwarded: {sql}"
    );
    ...
    assert!(
        sql_str.contains("ROW_GROUP_SIZE 250000"),
        "string row group size not forwarded: {sql_str}"
    );

    // Absent or zero => omit it, leaving DuckDB's default.
    let sql_none = build_parquet_sink(&serde_json::json!({"path": "out.parquet"}), "input");
    assert!(
        !sql_none.contains("ROW_GROUP_SIZE"),
        "must not emit a default: {sql_none}"
    );
    ...
    assert!(
        !sql_zero.contains("ROW_GROUP_SIZE"),
        "zero must be ignored: {sql_zero}"
    );
}
...
fn parquet_sink_partition_guard() {
    ...
    assert!(
        guarded.contains("PARTITION_BY (\"sender\", \"receiver\")"),
        "{guarded}"
    );
    assert!(
    ...
    assert!(
        !unlimited.contains("error("),
        "cap 0 must skip the guard: {unlimited}"
    );

    // No partitioning => no guard, plain source.
    let plain = build_parquet_sink(&serde_json::json!({"path": "out.parquet"}), "input");
    assert!(
        !plain.contains("approx_count_distinct") && !plain.contains("error("),
        "{plain}"
    );
}
...
fn cloud_csv_sink_honors_options_but_not_partitionby() {
    ...
    assert!(
        sql.contains("'s3://b/out.csv'"),
        "must write to the cloud path, got: {}",
        sql
    );
}
...
fn csv_windows_1252_encoding_is_remapped_to_cp1252() {
    ...
    assert!(
        sql.contains("encoding='CP1252'"),
        "windows-1252 must remap to CP1252, got: {}",
        sql
    );
    ...
    assert!(
        latin.contains("encoding='latin-1'"),
        "latin-1 must pass through, got: {}",
        latin
    );
}
...
fn db_sink_unknown_mode_fails_loud_not_destructive_overwrite() {
    ...
    .unwrap();
    assert!(
        ok.contains("DROP TABLE IF EXISTS"),
        "overwrite stays a recreate, got: {}",
        ok
    );
}
...
fn merge_mode_emits_partial_column_merge() {
    ...
    assert!(
        sql.contains(
            "WHEN MATCHED THEN UPDATE SET \"a\" = src.\"a\", \"b\" = src.\"b\" WHEN NOT MATCHED"
        ),
...
fn aggregate_missing_function_on_named_column_fails_loud() {
    ...
    let err = build_aggregate(
        ...
        .unwrap_err();
    assert!(
        err.contains("needs a function"),
        "named column without function must fail, got: {}",
        err
    );
...
fn kafka_offset_latest_maps_to_the_latest_sentinel() {
    ...
    assert_eq!(
        spec.start_offset, -2,
        "offset=latest must map to the -2 sentinel"
    );
}
...
fn quack_source_emits_attach_with_secret() {
    ...
    assert!(
        src_sql.contains("TYPE QUACK"),
        "wrong SECRET type: {}",
        src_sql
    );
    assert!(
        src_sql.contains("'super_secret'"),
        "token not in SECRET: {}",
        src_sql
    );
    assert!(
    ...
    assert!(
        src_sql.contains("AS duckle_src"),
        "wrong alias: {}",
        src_sql
    );
    assert!(
        src_sql.contains("READ_ONLY"),
        "missing READ_ONLY: {}",
        src_sql
    );
    assert!(
...
fn zip_arrays_to_table_pivots_to_real_columns() {
    ...
    let zip = compiled
        ...
        .expect("zip stage");
    assert!(
        zip.sql.contains("PIVOT"),
        "zip must emit a PIVOT: {}",
        zip.sql
    );
    assert!(
...
fn excel_source_honors_declared_schema() {
    ...
    assert!(
        !plain.contains("all_varchar"),
        "plain must not force all_varchar: {}",
        plain
    );
    ...
    assert!(
        typed.contains("all_varchar = true"),
        "typed must read all_varchar: {}",
        typed
    );
    assert!(
        typed.contains("CAST(\"id\" AS BIGINT)"),
        "id cast missing: {}",
        typed
    );
    assert!(
